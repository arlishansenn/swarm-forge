...
**这是 `arlishansenn/swarm-forge` 的 `four-pack` 分支，upstream 是
`unclebob/swarm-forge`。** 这条分支是**交付物**：`get-swarm-forge four-pack` 只从
这里取 `swarm` 与 `swarmforge/`（conf、roles、pack 专属 constitution articles），
本文件与 `test-swarm-launcher.sh` 都不会被装进用户的项目。

在这条分支上只会做一件事：**跟 `upstream/four-pack` 同步**。下面几节都是围绕那件事的。
...
**同步完还要跑一遍 RED 探针**：把 `swarm` 换成 `upstream/four-pack` 的版本再跑，
**必须有 7 个断言变红**。那里跑绿了，就说明 fork 的 launcher 已经被 upstream 的悄悄
换掉了。

## 同步与开 PR

只跟 `upstream/four-pack` 同步。**绝不把 `main` 合进来，也绝不把这条分支合进
