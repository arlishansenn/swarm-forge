...
**PR body 的四问由模型写，`Closes #N` 这些字段由脚本拼。** 在 issue #118 之前 body 是
四行 `printf`，podsum#149 就是那个样子——没有一个字是模型写的，`github-workflow.md` 的四问
对这条路径**从来没生效过**。现在分工是：脚本拼下游要解析的字段（`Closes #N`、`task:`、
`commit:`、`completed_at:`），模型写四个 `##` 小节（改了什么、怎么验证的、刻意没动、
我自己拿的主意）。
