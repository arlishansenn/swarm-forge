...
**Reason**: 它的两个 scenario 主体都在被删的路径上——一个点名 `onboard project`，另一个以
Pack 路径 managed project 的 `./swarm` first-run bootstrap 为主体。决定本身不失效，改由下面
那条以 forge 为主体的 requirement 承载。**这里写成 REMOVED + ADDED 而不是 MODIFIED，是因为
要显式丢掉一个 scenario；MODIFIED 会被 `openspec validate` 拦下，那道拦截正是为了不让 scenario
被静默丢掉。**
