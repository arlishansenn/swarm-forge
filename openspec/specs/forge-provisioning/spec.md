# forge-provisioning Specification

## Purpose
从一个空目录到一个跑着的 forge，外加它里面第一个跑着的 project —— 即 upstream README 的
第 2–4 步。本 capability 定的是安装来源、安装后的来源自证、三个阶段各自的可续跑语义、
两次 readiness 等待（tmux socket 与 dashboard HTTP），以及 New Project 经由 dashboard
本地 HTTP API 的路径。第 5 步 New Task 不在其内，那是 `issue-to-pr-pipeline` 的地盘。
